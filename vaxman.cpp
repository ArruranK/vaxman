...
#include <math.h>
using namespace std;
#define INITIAL_MONSTERS 4
#define MAX_MONSTER 128
...
Monster *head;
int monstersAlive = INITIAL_MONSTERS;
...
void resetGame()
{
    ...
    monster1->position = new float[3]{10.5, 8.5, 1.0};
    monster2->position = new float[3]{13.5, 1.5, 2.0};
    monster3->position = new float[3]{4.5, 6.5, 3.0};
    monster4->position = new float[3]{2.5, 13.5, 4.0};

    monster1->next = monster2;
    monster2->next = monster3;
    monster3->next = monster4;
    monster4->next = NULL;
    monster4->prev = monster3;
    monster3->prev = monster2;
    monster2->prev = monster1;
    monster1->prev = NULL;

    head = monster1;

    monstersAlive = INITIAL_MONSTERS;
    points = 0;
    for (int i = 0; i < 256; i++)
    {
        keyStates[i] = false;
...
void gameOver()
{
    ...
    Monster *currMonster = head;
    while (currMonster != NULL)
    {
        int monsterX = (int)(currMonster->position[0]);
        int monsterY = (int)(currMonster->position[1]);
        if (pacmanX == monsterX && pacmanY == monsterY)
        {
            if (currMonster == head)
            {
                head = currMonster->next;
                if (head != NULL)
                {
                    head->prev = NULL;
                }
            }
            else if (currMonster->next != NULL)
            {
                currMonster->prev->next = currMonster->next;
                currMonster->next->prev = currMonster->prev;
            }
            else
            {
                currMonster->prev->next = NULL;
            }

            monstersAlive = monstersAlive - 1;
        }
        currMonster = currMonster->next;
    }

    if (points == 106 || monstersAlive >= MAX_MONSTER)
    {
        over = true;
    }
}
...
void display()
{
    ...
    if (replay)
    {
        if (!over)
        {
            int time = (int)(((float)clock()) / CLOCKS_PER_SEC);
            if (time - startTime == 30)
            {
                ...
                while (currMonster != NULL)
                {
                    ...
                    newMonster->prev = NULL;
                    head->prev = newMonster;
                    newMonster->position = new float[3]{currMonster->position[0], currMonster->position[1], currMonster->position[2]};
                    head = newMonster;
                    monstersAlive = monstersAlive + 1;
                    currMonster = currMonster->next;
                }
            ...
            while (currMonster != NULL)
            {
                updateMonster(currMonster->position);
                currMonster = currMonster->next;
            }
            ...
            while (currMonster != NULL)
            {
                drawMonster(currMonster->position[0], currMonster->position[1], 0.0, 1.0, 1.0);
                currMonster = currMonster->next;
            }
        }
        else
...
{
    startTime = (int)(((float)clock()) / CLOCKS_PER_SEC);
